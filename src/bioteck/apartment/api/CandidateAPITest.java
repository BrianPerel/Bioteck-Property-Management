...
public class CandidateAPITest {
	...
	private Candidate c1 = Candidate.create("394-321-3454", "Francis", "John", "55, Granite Street, Hooksett, NH",
			"584-322-9965");
	...
	private void removeTestCandidate(String ssn) {
		setup();
		Response response = given().queryParam("searchany", ssn).expect().statusCode(200).log().ifError().when()
				.get("/candidates/query");

		List<Map<String, String>> candidatesList = response.jsonPath().getList("");

		given().pathParam("id", candidatesList.get(0).get("id")).expect().statusCode(204).log().ifError().when()
