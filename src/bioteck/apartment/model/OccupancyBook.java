...
public class OccupancyBook {
	...
	public void addRenterToApartment(Candidate renter, Apartment apt) {
		...
		DB.transact(s -> {
			List<Candidate> rr = s
					.createQuery("select c " + "from Candidate c " + "where c.socialSecurity = :cssn", Candidate.class)
					.setParameter("cssn", renter.getSocialSecurity()).getResultList();

			List<Apartment> aa = s
					.createQuery("select a " + "from Apartment a " + "where a.complex.name = :acn "
							+ "  and a.number = :an ", Apartment.class)
					.setParameter("acn", apt.getComplex().getName()).setParameter("an", apt.getNumber())
					.getResultList();
	...
	public void removeRenterFromApartment(Candidate renter, Apartment apt) {
		...
		DB.transact(s -> {
			s.createQuery("delete Candidate2Apartment c2a where " + "c2a.apartment = :acn and " + "c2a.renter = :cssn ")
					.setParameter("acn", apt).setParameter("cssn", renter).executeUpdate();
		});
	}
	...
	public Set<Candidate> rentersForApartment(Apartment a) {
		...
		List<Candidate2Apartment> ra = DB.query(s -> s
				.createQuery("select c2a " + "from Candidate2Apartment c2a "
						+ "where c2a.apartment.complex.name = :acn " + "  and c2a.apartment.number = :an ",
						Candidate2Apartment.class)
				.setParameter("acn", a.getComplex().getName()).setParameter("an", a.getNumber()).getResultList());
	...
	public Set<Candidate2Apartment> occupancySet() {
		...
		List<Candidate2Apartment> ca = DB
				.query(s -> s.createQuery("select c2a " + "from Candidate2Apartment c2a ", Candidate2Apartment.class)
						.getResultList());
	...
	public Map<Apartment, Double> calculateRentStatement(Candidate renter) {
		...
		List<Candidate2Apartment> cc = DB
				.query(s -> s
						.createQuery("select c2a " + "from Candidate2Apartment c2a "
								+ "where c2a.renter.socialSecurity = :cssn", Candidate2Apartment.class)
						.setParameter("cssn", renter.getSocialSecurity()).getResultList());

		// Do a simple calculation for rent: Evenly divide rent by the number of renters
		// in the apartment !z
		return cc.stream().map(g -> g.getApartment())
				.collect(Collectors.toMap(a -> a, a -> a.getRent() / this.rentersForApartment(a).size()));
	}
	...
	public double monthlyRevenue(ApartmentComplex property) {
		...
		List<Candidate2Apartment> ac = DB
				.query(s -> s
						.createQuery("select c2a " + "from Candidate2Apartment c2a "
								+ "where c2a.apartment.complex.name= :acname", Candidate2Apartment.class)
						.setParameter("acname", property.getName()).getResultList());
		return ac.stream().map(g -> g.getApartment()).collect(Collectors.toSet()).stream().mapToDouble(g -> g.getRent())
				.sum();
	}
